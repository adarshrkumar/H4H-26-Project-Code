...
struct WebSpatialApp: App {
    ...
    var body: some Scene {
        WindowGroup(
            ...
            if let spatialScene = SpatialApp.Instance.getScene(windowData) {
                SpatialSceneView(spatialScene: spatialScene)
            } else {
                let fallbackScene = SpatialApp.Instance.createScene(
                    startURL,
                    .window,
                    .visible,
                    app.getSceneOptions(windowData) ?? app.getSceneOptions()
                )
                SpatialSceneView(spatialScene: fallbackScene)
            }
        }
        defaultValue: {
